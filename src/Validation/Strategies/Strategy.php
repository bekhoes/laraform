        ...
        foreach ($rules as $key => $rule) {
            switch ($this->getRuleType($rule, $key)) {
                ...
                case 'language':
                    ...
                    if (!is_array($langRules)) {
                      $langRules = explode('|', $langRules);
                    }

                    foreach ($langRules as $langRule) {
                      $this->addRule($validator, $langRule, $name . '.' . $lang);
                    }
                    break;
        ...
        foreach ($rules as $key => $rule) {
            switch ($this->getRuleType($rule, $key)) {
                ...
                case 'language':
                    ...
                    if (!is_array($langRules)) {
                       $langRules = array_map(function($rule) {
                            return explode(':', $rule)[0];
                        }, explode('|', $langRules));
                    }

                    foreach ($langRules as $langRule) {
                      $this->addMessage($validator, $langRule, $name . '.' . $lang);
                    }
